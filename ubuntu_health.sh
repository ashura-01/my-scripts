...
# ==============================================================================
# UBUNTU SYSTEM HEALTH CHECK - MASTER EDITION v3.3
...
GRAY=$'\e[0;37m'
DKGRAY=$'\e[2;37m'
ORANGE=$'\e[38;5;215m'   # Whitish orange for Ubuntu
WARM_ORANGE=$'\e[38;5;216m' # Warmer orange
...
hdr() {
    local title="$1"
    local len=${#title}
    local padding=$(( 76 - len - 2 ))
    echo -e "\n${ORANGE}╔══════════════════════════════════════════════════════════════════════════════════╗${NC}"
    printf "${ORANGE}║${NC} ${ORANGE}${BOLD}⚡ %s${NC}%*s${ORANGE}${NC}\n" "$title" "$padding" ""
    echo -e "${ORANGE}╚══════════════════════════════════════════════════════════════════════════════════╝${NC}"
}
...
inf() { echo -e "  ${ORANGE}${BOLD} • ${NC} $*"; }
kv()  { printf "  ${ORANGE}${BOLD}%-22s${NC}${WHITE}%s${NC}\n" "$1" "$2"; }

# ── Gather Data ────────────────────────────────────────────────────────────────
os_name=$(lsb_release -ds 2>/dev/null | sed 's/^"//;s/"$//')
os_id=$(lsb_release -c 2>/dev/null | awk '{print $2}')
os_short=$(lsb_release -is 2>/dev/null || echo "LINUX")
kernel_ver=$(uname -r)
cpu_arch=$(uname -m)
...
printf "${ORANGE}"
echo "  ╔══════════════════════════════════════════════════════════════════════════════════╗"
printf "  ║          ${os_short^^}  ·  SYSTEM INTEGRITY & DIAGNOSTICS  v3.3%*s║\n" $(( 72 - ${#os_short} )) ""
echo "  ╚══════════════════════════════════════════════════════════════════════════════════╝"
printf "${NC}"

# ── Ubuntu Logo + Info ─────────────────────────────────────────────────────────
cat << EOF
  ${WARM_ORANGE}⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⠋⠉⠁⠀⠀⠀⠀⠈⠉⠙⠛⠿⣿⣿⣿⣿⣿⣿⣿⣿${NC}  ${WARM_ORANGE}╔═════════════════════════════════════════╗${NC}
  ${WARM_ORANGE}⣿⣿⣿⣿⣿⠿⠋⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠙⠿⣿⣿⣿⣿⣿⣿⣿${NC}  ${WARM_ORANGE}║${NC}  ${WHITE}${BOLD}$(printf '%-39s' "  ${hostname_val}  @  ${os_id:-ubuntu}")${NC}${WARM_ORANGE} ${NC}
  ${WARM_ORANGE}⣿⣿⣿⡟⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⣾⣿⣦⠀⠀⠀⠈⢻⣿⣿⣿${NC}  ${WARM_ORANGE}╚═════════════════════════════════════════╝${NC}
  ${WARM_ORANGE}⣿⣿⠏⠀⠀⠀⠀⠀⠀⠀⠀⢠⣶⣶⣾⣷⣶⣆⠸⣿⣿⡟⠀⠀⠀⠀⠀⠹⣿⣿${NC}       ${WARM_ORANGE}${BOLD}OS${NC}         ${WHITE}${os_name}${NC}
  ${WARM_ORANGE}⣿⠃⠀⠀⠀⠀⠀⠀⣠⣾⣷⡈⠻⠿⠟⠻⠿⢿⣷⣤⣤⣄⠀⠀⠀⠀⠀⠀⠘⣿${NC}      ${WARM_ORANGE}${BOLD}Kernel${NC}     ${WHITE}${kernel_ver}${NC}
  ${WARM_ORANGE}⡏⠀⠀⠀⠀⠀⠀⣴⣿⣿⠟⠁⠀⠀⠀⠀⠀⠀⠈⠻⣿⣿⣦⠀⠀⠀⠀⠀⠀⢹${NC}      ${WARM_ORANGE}${BOLD}Shell${NC}      ${WHITE}${shell_name} ${shell_ver}${NC}
  ${WARM_ORANGE}⠁⠀⠀⢀⣤⣤⡘⢿⣿⡏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢹⣿⣿⡇⠀⠀⠀⠀⠀⠈${NC}      ${WARM_ORANGE}${BOLD}Uptime${NC}     ${WHITE}${uptime_str}${NC}
  ${WARM_ORANGE}⠀⠀⠀⣿⣿⣿⡇⢸⣿⡁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢈⣉⣉⡁⠀⠀⠀⠀⠀⠀${NC}      ${WARM_ORANGE}${BOLD}Packages${NC}   ${WHITE}${pkgs} (dpkg)${NC}
  ${WARM_ORANGE}⡀⠀⠀⠈⠛⠛⢡⣾⣿⣇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣸⣿⣿⡇⠀⠀⠀⠀⠀⢀${NC}      ${WARM_ORANGE}${BOLD}Terminal${NC}   ${WHITE}${term}${NC}
  ${WARM_ORANGE}⣇⠀⠀⠀⠀⠀⠀⠻⣿⣿⣦⡀⠀⠀⠀⠀⠀⠀⢀⣴⣿⣿⠟⠀⠀⠀⠀⠀⠀⣸${NC}      ${WARM_ORANGE}${BOLD}DE/WM${NC}      ${WHITE}${de_wm}${NC}
  ${WARM_ORANGE}⣿⡄⠀⠀⠀⠀⠀⠀⠙⢿⡿⢁⣴⣶⣦⣴⣶⣾⡿⠛⠛⠋⠀⠀⠀⠀⠀⠀⢠⣿${NC}      ${WARM_ORANGE}${BOLD}IP${NC}         ${WHITE}${ip_local}${NC}
  ${WARM_ORANGE}⣿⣿⣆⠀⠀⠀⠀⠀⠀⠀⠀⠘⠿⠿⢿⡿⠿⠏⢰⣿⣿⣧⠀⠀⠀⠀⠀⣰⣿⣿${NC}
  ${WARM_ORANGE}⣿⣿⣿⣧⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⢿⣿⠟⠀⠀⠀⢀⣼⣿⣿⣿${NC}
  ${WARM_ORANGE}⣿⣿⣿⣿⣿⣶⣄⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣠⣶⣿⣿⣿⣿⣿${NC}
  ${WARM_ORANGE}⣿⣿⣿⣿⣿⣿⣿⣿⣶⣤⣄⣀⡀⠀⠀⠀⠀⢀⣀⣠⣤⣶⣿⣿⣿⣿⣿⣿⣿⣿${NC}
...
echo -e "\n  ${WARM_ORANGE}${BOLD}Filesystem Mounts${NC}  ${DKGRAY}(tmpfs/devtmpfs hidden)${NC}"
printf "  ${WARM_ORANGE}${BOLD}%-22s %6s %6s %6s %5s  %s${NC}\n" \
  "Device" "Size" "Used" "Avail" "Use%" "Mount"
echo -e "  ${WARM_ORANGE}$(printf '%.0s─' {1..75})${NC}"
...
echo -e "\n  ${WARM_ORANGE}${BOLD}Block Devices${NC}"
...
if command -v dmidecode &>/dev/null; then
    ...
    if [ -n "$dimm" ]; then
        echo -e "\n  ${WARM_ORANGE}${BOLD}  DIMM Slots (DMI):${NC}"
...
sub "NETWORK"
printf "  ${WARM_ORANGE}${BOLD}%-16s %-24s %-20s %s${NC}\n" "Interface" "Address" "MAC" "State"
echo -e "  ${WARM_ORANGE}$(printf '%.0s─' {1..72})${NC}"
...
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.1] Package Database Integrity${NC}"
...
# 4.2 Pending updates
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.2] Pending System Updates${NC}"
...
# 4.3 Failed services
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.3] Systemd Service Status${NC}"
...
# 4.4 Journal errors
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.4] Journal Critical Errors (this boot)${NC}"
...
# 4.5 dmesg errors
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.5] Disk I/O Errors (dmesg)${NC}"
...
# 4.6 Load
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.6] System Load${NC}"
...
# Top 5 processes
echo -e "\n  ${WARM_ORANGE}${BOLD}  Top 5 by CPU:${NC}"
printf "  ${WARM_ORANGE}%-10s %-28s %s${NC}\n" "PID" "Process" "CPU%"
echo -e "  ${WARM_ORANGE}$(printf '%.0s─' {1..50})${NC}"
ps -eo pid,comm,%cpu --sort=-%cpu --no-headers 2>/dev/null | head -5 | \
while read -r pid comm pcpu; do
    printf "  ${WHITE}%-10s${NC} ${WARM_ORANGE}%-28s${NC} ${WARM_ORANGE}%s%%${NC}\n" \
      "$pid" "$comm" "$pcpu"
done

# 4.7 Disk space
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.7] Root Partition Space${NC}"
...
# 4.8 Swap
echo -e "\n  ${WARM_ORANGE}${BOLD}[4.8] Swap / Virtual Memory${NC}"
...
# 5.1 Thermals
echo -e "\n  ${WARM_ORANGE}${BOLD}[5.1] Thermal Status${NC}"
thermal_found=false
...
echo -e "\n  ${WARM_ORANGE}${BOLD}[5.3] Power Management${NC}"
...
# 5.4 ECC
echo -e "\n  ${WARM_ORANGE}${BOLD}[5.4] Memory ECC Errors${NC}"
...
# 5.5 SMART
echo -e "\n  ${WARM_ORANGE}${BOLD}[5.5] Disk SMART Health${NC}"
...
# 5.6 CPU steal
echo -e "\n  ${WARM_ORANGE}${BOLD}[5.6] CPU Steal & IRQ${NC}"
...
echo ""
printf "${WARM_ORANGE}╔══════════════════════════════════════════════════════════════════════════════════╗\n${NC}"
printf "${WARM_ORANGE}║${NC}  ${WARM_ORANGE}${BOLD}✔  UBUNTU DIAGNOSTIC COMPLETE${NC}  ${DKGRAY}%s${NC}%*s${WARM_ORANGE}${NC}\n" \
